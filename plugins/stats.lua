...
local function run(msg, matches)
  if matches[1]:lower() == 'teleBad' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /telebad ")
    return about
  end 
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "telebad" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "^[!/]([Ss]tatslist)$",
    "^[!/]([Ss]tats) (group) (%d+)",
    "^[!/]([Ss]tats) (teld)",-- Put everything you like :
	"^[!/]([Tt]elebad)"-- Put everything you like :)
    }, 
  run = run
}
